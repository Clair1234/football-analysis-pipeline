...
class MultiAgentTransformer(nn.Module):
    ...
    def get_memory_usage(self):
        """Get approximate memory usage of the model"""
        total_params = sum(p.numel() for p in self.parameters())
        param_memory = total_params * 4  # Assuming float32, 4 bytes per param
    ...
    def compute_nash_equilibrium(self, embeddings, team_ids):
        """ADD: Game theoretic analysis"""
        if team_ids is None:
            return {}
        
        results = {}
        for batch_idx in range(embeddings.shape[0]):
            player_embeddings = embeddings[batch_idx, :-1, :]  # Exclude ball
            batch_team_ids = team_ids[batch_idx]
            
            team_a_embeddings = player_embeddings[batch_team_ids == 0]
            team_b_embeddings = player_embeddings[batch_team_ids == 1]
            
            if len(team_a_embeddings) > 0 and len(team_b_embeddings) > 0:
                # Team coherence (cooperation within teams)
                team_a_coherence = F.cosine_similarity(
                    team_a_embeddings.unsqueeze(1), 
                    team_a_embeddings.unsqueeze(0), 
                    dim=-1
                ).mean()
                
                team_b_coherence = F.cosine_similarity(
                    team_b_embeddings.unsqueeze(1), 
                    team_b_embeddings.unsqueeze(0), 
                    dim=-1
                ).mean()
                
                # Competitive dynamics (inter-team competition)
                competitive_matrix = torch.matmul(team_a_embeddings, team_b_embeddings.transpose(-2, -1))
                competitive_balance = competitive_matrix.mean()
                
                # Nash stability estimate
                eigenvals = torch.linalg.eigvals(competitive_matrix)
                nash_stability = torch.mean(torch.real(eigenvals)).item()
                
                results[f'batch_{batch_idx}'] = {
                    'team_a_coherence': team_a_coherence.item(),
                    'team_b_coherence': team_b_coherence.item(),
                    'competitive_balance': competitive_balance.item(),
                    'nash_stability': nash_stability
                }
        
        return results

    def get_comprehensive_analysis(self, player_states, ball_states, team_ids=None):
        """ADD: Combined analysis method"""
        # Enable gradients for importance analysis
        player_states.requires_grad_(True)
        ball_states.requires_grad_(True)
        ...
        embeddings, metadata = self.extract_embeddings(player_states, ball_states, team_ids)
        
        # Get attention weights
        all_attention_weights = self.get_all_attention_weights()
        
        # Compute gradients for importance
        loss = player_preds.sum() + ball_preds.sum()
        loss.backward()
        
        # Game theoretic analysis
        game_theory_results = self.compute_nash_equilibrium(embeddings, team_ids)
        
        return {
            'predictions': {
                'players': player_preds.detach(),
                'ball': ball_preds.detach()
            },
            'embeddings': embeddings,
            'tactical_context': metadata['tactical_context'],
            'attention_weights': all_attention_weights,
            'gradient_importance': {
                'players': player_states.grad.abs() if player_states.grad is not None else None,
                'ball': ball_states.grad.abs() if ball_states.grad is not None else None
            },
            'game_theory': game_theory_results,
            'metadata': metadata
        }
        
    def safe_comprehensive_analysis(self, player_states, ball_states, team_ids=None): #not working
        """Safe version that doesn't modify input tensors"""
        # Clone inputs to avoid modifying originals
        player_states_copy = player_states.clone().detach().requires_grad_(True)
        ball_states_copy = ball_states.clone().detach().requires_grad_(True)
        
        # Temporarily enable attention storage for analysis
        self.enable_attention_storage()
        
        try:
            return self.get_comprehensive_analysis(player_states_copy, ball_states_copy, team_ids)
        finally:
            # Clean up
            if self.training:
                self.disable_attention_storage()
...
class SoccerTrainer:
    ...
    def check_model_behavior_v0(self, batch):
        """
        Check if model behaves differently in train vs eval mode
        """
        print(f"\n=== CHECKING MODEL BEHAVIOR ===")
        
        player_states = batch['player_states'].to(self.device)  # Move to the same device as the model
        ball_states = batch['ball_states'].to(self.device)      # Move to the same device as the model
        team_ids = batch.get('team_ids', None)
        
        # Move team_ids to the same device if it exists
        if team_ids is not None:
            team_ids = team_ids.to(self.model.device)

        # Test in training mode
        self.model.train()
        with torch.no_grad():
            player_preds_train, ball_preds_train = self.model.forward(player_states, ball_states, team_ids)
        
        # Test in eval mode
        self.model.eval()
        with torch.no_grad():
            player_preds_eval, ball_preds_eval = self.model.forward(player_states, ball_states, team_ids)
        
        # Compare outputs
        player_diff = torch.abs(player_preds_train - player_preds_eval).mean()
        ball_diff = torch.abs(ball_preds_train - ball_preds_eval).mean()
        
        print(f"Player predictions difference (train vs eval): {player_diff:.6f}")
        print(f"Ball predictions difference (train vs eval): {ball_diff:.6f}")
        
        if player_diff > 1e-6 or ball_diff > 1e-6:
            print("WARNING: Model outputs differ significantly between train/eval modes!")
            print("This could be due to dropout, batch norm, or other training-specific layers")
        
        return player_diff, ball_diff
    
    def check_model_behavior_v1(self, batch):
        """
        Check if model behaves consistently across different modes
        """
        print(f"\n=== CHECKING MODEL BEHAVIOR ===")
        ...
        # Test in training mode (but with no_grad)
        ...
        self.model.eval()
        with torch.no_grad():
            player_preds_regular, ball_preds_regular = self.model.forward(
                player_states, ball_states, team_ids
            )
        ...
        # Compare outputs
        player_diff_train_eval = torch.abs(player_preds_train - player_preds_eval).mean()
        ball_diff_train_eval = torch.abs(ball_preds_train - ball_preds_eval).mean()
        
        player_diff_eval_regular = torch.abs(player_preds_eval - player_preds_regular).mean()
        ball_diff_eval_regular = torch.abs(ball_preds_eval - ball_preds_regular).mean()
        
        print(f"Player predictions difference (train vs eval): {player_diff_train_eval:.8f}")
        print(f"Ball predictions difference (train vs eval): {ball_diff_train_eval:.8f}")
        print(f"Player predictions difference (eval vs regular): {player_diff_eval_regular:.8f}")
        print(f"Ball predictions difference (eval vs regular): {ball_diff_eval_regular:.8f}")
        
        tolerance = 1e-6
        
        if player_diff_train_eval > tolerance or ball_diff_train_eval > tolerance:
            print("WARNING: Model outputs differ significantly between train/eval modes!")
            print("This could be due to dropout, batch norm, or other training-specific layers")
            return False
        
        if player_diff_eval_regular > tolerance or ball_diff_eval_regular > tolerance:
            print("WARNING: Model outputs differ between eval methods!")
            return False
        
        print("Model behavior is consistent across different modes")
        return True
    ...
    def analyze_model_safely(self, dataloader: DataLoader, num_samples: int = 1):
        """Safe model analysis that doesn't interfere with training memory"""
        was_training = self.model.training
        self.model.eval()
        ...
        try:
            with torch.no_grad():
                for i, batch in enumerate(dataloader):
                    ...
                    # Use safe comprehensive analysis if available
                    if hasattr(self.model, 'safe_comprehensive_analysis'):
                        analysis = self.model.safe_comprehensive_analysis(
                            player_states, ball_states, team_ids
                        )
                        print(f"Analysis for sample {i+1}:")
                        print(f"- Player prediction shape: {analysis['predictions']['players'].shape}")
                        print(f"- Ball prediction shape: {analysis['predictions']['ball'].shape}")
                        if 'game_theory' in analysis:
                            print(f"- Game theory results: {len(analysis['game_theory'])} batches analyzed")
    ...
    def prepare_game_theoretic_analysis(self, dataloader: DataLoader):
        """
        Prepare data for game-theoretic analysis
        Extract utility-relevant features and strategic interactions
        """
        self.model.eval()
        ...
        with torch.no_grad():
            for batch_idx, batch in enumerate(dataloader):
                if batch_idx >= 100:  # Limit for computational efficiency
                    break
                ...
                player_preds, ball_preds = self.model.forward(player_states, ball_states, team_ids)
                embeddings, metadata = self.model.extract_embeddings(player_states, ball_states, team_ids)
                
                # Calculate utility-relevant features
                batch_strategic_data = {
                    'positions': metadata['positions'].cpu(),
                    'predicted_positions': player_preds.cpu(),
                    'ball_position': metadata['ball_position'].cpu(),
                    'ball_prediction': ball_preds.cpu(),
                    'embeddings': embeddings.cpu(),
                    'team_assignments': team_ids.cpu() if team_ids is not None else None,
                    # Add space control, positional advantage metrics here
                    'spatial_features': self._compute_spatial_features(metadata, player_preds, ball_preds)
                }
                
                strategic_data.append(batch_strategic_data)
        
        return strategic_data

    def _compute_spatial_features(self, metadata, player_preds, ball_preds):
        """Helper to compute spatial control and advantage metrics"""
        # Simplified version - expand based on your game-theoretic requirements
        positions = metadata['positions']  # (batch, players, 2)
        
        spatial_features = {
            'team_centroids': self._compute_team_centroids(positions, metadata.get('team_ids')),
            'spread_metrics': torch.std(positions, dim=1),  # Team spread
            'ball_distances': torch.norm(positions - metadata['ball_position'].unsqueeze(1), dim=-1)
        }
        
        return spatial_features

    def _compute_team_centroids(self, positions, team_ids):
        """Compute team centroids for formation analysis"""
        if team_ids is None:
            return None
            
        centroids = []
        for batch_idx in range(positions.shape[0]):
            team_a_pos = positions[batch_idx][team_ids[batch_idx] == 0]
            team_b_pos = positions[batch_idx][team_ids[batch_idx] == 1]
            
            centroids.append({
                'team_a': team_a_pos.mean(dim=0) if len(team_a_pos) > 0 else torch.zeros(2),
                'team_b': team_b_pos.mean(dim=0) if len(team_b_pos) > 0 else torch.zeros(2)
            })
        
        return centroids

# Example usage and additional utility functions
def analyze_spatio_temporal_patterns(model: MultiAgentTransformer, 
                                   dataloader: DataLoader, 
                                   device: str = 'cuda'):
    """
    Comprehensive analysis of learned spatio-temporal patterns
    """
    model.eval()
    visualizer = AttentionVisualizer(model.num_players)
    
    print("=== Multi-Agent Temporal Transformer Analysis ===")
    print(f"Model Architecture:")
    print(f"- Input: {model.num_players} players + 1 ball = {model.num_agents} agents")
    print(f"- Transformer dimension: {model.d_model}")
    print(f"- Attention heads: {model.nhead}")
    print(f"- Encoder layers: {len(model.encoder_layers)}")
    print(f"- Forecast horizon: {model.forecast_horizon}") # have to deal with this 
    ...
    with torch.no_grad():
        for i, batch in enumerate(dataloader):
            ...
            print(f"  Players: {player_states.shape}")
            print(f"  Ball: {ball_states.shape}")
            
            # Forward pass
            player_preds, ball_preds = model(player_states, ball_states)
            
            print(f"Output shapes:")
            print(f"  Player predictions: {player_preds.shape}")
            print(f"  Ball predictions: {ball_preds.shape}")
            
            # Analyze attention patterns for each layer
            all_attention_weights = model.get_all_attention_weights()
            
            for layer_idx, attention_weights in enumerate(all_attention_weights):
                if attention_weights is not None:
                    print(f"\nLayer {layer_idx} attention analysis:")
                    print(f"  Attention shape: {attention_weights.shape}")
                    
                    # Analyze ball-player interactions
                    analyze_ball_player_attention(attention_weights, layer_idx, visualizer)
                    
                    # Analyze inter-player attention
                    analyze_inter_player_attention(attention_weights, layer_idx, visualizer)

def analyze_ball_player_attention(attention_weights: torch.Tensor, 
                                layer_idx: int, 
                                visualizer: AttentionVisualizer):
    """Analyze attention patterns between ball and players"""
    
    # Average over batch and heads
    avg_attention = attention_weights.mean(dim=(0, 1))  # (seq_len, agents, agents)
    
    # Take last timestep
    final_attention = avg_attention[-1].cpu().numpy()  # (agents, agents)
    
    ball_idx = 22  # Ball is agent 22
    
    # Ball attention to players
    ball_to_players = final_attention[ball_idx, :22]  # Ball's attention to players
    
    # Players attention to ball
    players_to_ball = final_attention[:22, ball_idx]  # Players' attention to ball
    
    print(f"    Ball-Player Interactions (Layer {layer_idx}):")
    print(f"    - Ball attention to players (avg): {ball_to_players.mean():.4f}")
    print(f"    - Players attention to ball (avg): {players_to_ball.mean():.4f}")
    print(f"    - Most attended player by ball: P{np.argmax(ball_to_players)}")
    print(f"    - Player most attending to ball: P{np.argmax(players_to_ball)}")

def analyze_inter_player_attention(attention_weights: torch.Tensor, 
                                 layer_idx: int, 
                                 visualizer: AttentionVisualizer):
    """Analyze attention patterns between players"""
    
    # Average over batch and heads
    avg_attention = attention_weights.mean(dim=(0, 1))  # (seq_len, agents, agents)
    
    # Take last timestep and only player-to-player attention
    final_attention = avg_attention[-1, :22, :22].cpu().numpy()  # (22, 22)
    
    # Remove self-attention (diagonal)
    np.fill_diagonal(final_attention, 0)
    
    # Find strongest inter-player connections
    max_attention_idx = np.unravel_index(np.argmax(final_attention), final_attention.shape)
    max_attention_value = final_attention[max_attention_idx]
    
    print(f"    Inter-Player Interactions (Layer {layer_idx}):")
    print(f"    - Average inter-player attention: {final_attention.mean():.4f}")
    print(f"    - Strongest connection: P{max_attention_idx[0]} → P{max_attention_idx[1]} ({max_attention_value:.4f})")
    
    # Analyze potential team formations (first 11 are team A, next 11 are team B)
    team_a_attention = final_attention[:11, :11].mean()
    team_b_attention = final_attention[11:22, 11:22].mean()
    cross_team_attention = (final_attention[:11, 11:22].mean() + final_attention[11:22, :11].mean()) / 2
    
    print(f"    - Team A internal attention: {team_a_attention:.4f}")
    print(f"    - Team B internal attention: {team_b_attention:.4f}")
    print(f"    - Cross-team attention: {cross_team_attention:.4f}")